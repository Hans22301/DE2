...
ISR(TIMER1_OVF_vect)
{
    ...
    if (sla < 120)
    {
        if (ack == 0)
        {
        uart_puts("Port found at:");
        uart_puts(itoa(sla,string,10));
        uart_puts("\r\n");
        }
        sla++;
    }
    */
    ...
    uart_puts("Humidity:");
    uart_puts(itoa(air.humid_int,string,10));
    uart_puts(".");
    ...
    uart_puts(itoa(air.humid_dec,string,10));
    uart_puts("\n");
    ...
    uart_puts(itoa(air.temp_int,string,10));
    uart_puts(".");
    ...
    uart_puts(itoa(air.temp_dec,string,10));
    uart_puts("\n");
